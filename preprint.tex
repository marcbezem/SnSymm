...
\author{Pierre Cagne\thanks{Universitetet i Bergen} \and Ulrik Buchholtz\thanks{Technische Universit\"at Darmstadt} \and Nicolai
...
\[
  ...
  \end{tikzcd}
\]
If $A$ is pointed, then we take the base point of $A*B$ to be $\inl(*_A)$.
...
\[
  ...
  \end{tikzcd}
\]
again with $\inl(*_A)$ as the base point.
In this case, we also have the \emph{wedge inclusion}
$i : A \vee B \ptdto A \times B$
...
\begin{align*}
  ...
  H(\inr b) &\defequi \beta_\ast \cdot \ap{\beta}(\sigma_B\,b)^{-1} \cdot
              \miniast\beta\alpha : \alpha(N) = \ast_X
\end{align*}
...
\[
  \begin{tikzcd}
    \beta(N)  \ar[ddd,"{\refl{}}"']\ar[r,"{\ap{\beta}(\sigma_B\,b)}"] &[10pt]
    \beta(N)  \ar[r,"{\miniast\alpha\beta}"] &[-15pt]
    \alpha(N) \ar[r,"{\ap{\alpha}(\sigma_A\,a)}"] &[10pt]
    ...
    &
    &
%\beta(N) \ar[d,"{H(\inl a)}"'] \ar[rr,"{\ap{\rho_\beta(\alpha)}(\glue(a,b))}"]
    &
    &
%\alpha(N) \ar[d,"{H(\inr b)}"]
    &
    &
    \beta(N)  \ar[d,"{\ap{\beta}(\sigma_B\,b)^{-1}}"]
    \\[10pt]
    &
    &
%\beta(N) \ar[rr,"{\ap{g}(\glue(a,b))}"]
    &
    &
%\ast_X
    &
    ...
    \\[-10pt]
    \beta(N)  \ar[r,"{\ap{\beta}(\sigma_B\,b)}"'] &
    \beta(N)  \ar[r,"{\miniast\alpha\beta}"'] &
    \alpha(N) \ar[r,"{\ap{\alpha}(\sigma_A\,a)}"'] &
    \alpha(N) \ar[r,"{\miniast\beta\alpha}"'] &
    \beta(N)  \ar[r,"{\ap{\beta}(\sigma_B\,b)^{-1}}"'] &
...
\begin{theorem} \label{thm:Sp-sym=Z/2}
  $\fgr (\Sp = \Sp) = \ZZ/2 \ZZ$
...
\begin{proof}
  ...
  specialized to $X \jdeq \Sp$, $q=1$ and $\beta\jdeq (\id_\Sp, \refl N): \Sp
  \ptdto \Sp$. The element in $\hgr 2(\Sp)$ represented by $(\id_\Sp, \refl N)$
  is:
  \begin{equation}
...
\appendix
\section{Appendix: Wild Categories}
%\label{sec:appendix}
%
