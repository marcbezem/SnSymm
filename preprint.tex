...
\end{displaymath}
which is pointed by the path $\varpi_{f}:{f_0} \cdot (\ap f (\inv
\mrd(a)\mrd(a))) \cdot \inv{f_0} = \refl{b}$ given by path algebra operations.
...
%%
%%\todo[inline]{Nicolai: Here, we should prove that id and -id are elements of the two components (i.e.\ in different components). My plan was to show that the two wild functors $\susp$ and $-\susp$ commute. But it's trickier than I thought. Maybe we don't do this?}
%%
...
\begin{definition}
  A \emph{wild category} is a dependent tuple $(\cat C, \hom( , ), \circ, \id, \alpha,
  \iota)$ where:
  \begin{align*}
    \cat C&: \UU \\
  ...
  \label{defn:wild-cat}
\end{definition}
One makes the abuse to denote a category by only its carrier type $\cat C$ when
all the remaining data are clear from context.

\begin{example}
  The types $\UU$ together with function types, identity functions and the
  usual composition, is a wild category. The elements $\alpha$ and $\iota$ are
...
\begin{definition}
  ...
  \begin{align*}
    ...
    c &: \prod_{a,b,c:\cat C}\prod_{f:\hom(a,b)}\prod_{g:\hom(a,c)}F_1(g\circ f) = F_1(g) \circ F_1(f)\\
...
\begin{example}~\label{ex:loop-sus-wild-functors}%
  \begin{enumerate}
    \item There is a wild functor $\loopspace \null$ which maps a pointed type $A$
      ...
      \begin{displaymath}
        \loopspace\null (f)(\refl a) =  f_0 \cdot \ap f (\refl a) \cdot \inv{f_0}
        = f_0 \cdot \refl {f(a)} \inv{f_0}
        = \refl b.
      \end{displaymath}
...
%%
    \item There is a wild functor $\susp \null$ which maps a pointed type $A$
      ...
      straightforward through degenerate inductions on the supsension, and a
...
\begin{definition}
  ...
  \begin{displaymath}
    \eta : \prod_{A:\UUptd} A \to R \circ L(A), \quad
    \epsilon : \prod_{A:\UUptd} L\circ R (A) \to A
  \end{displaymath}
  together with elements
  \begin{gather*}
    \mathrm{nat}_\eta : \prod_{A,B:\UUptd} \prod_{f:A\ptdto B} (R\circ L)(f) \circ \eta(A) = \eta(B) \circ f, \\
    \mathrm{nat}_\epsilon : \prod_{A,B:\UUptd} \prod_{f:A\ptdto B} f \circ \epsilon(A) = (L\circ R)(f) \circ \epsilon(B).
  \end{gather*}
  and
  \begin{displaymath}
    \mathrm{rtr}_{\eta,\epsilon} : \prod_{A:\UUptd} R(\epsilon(A)) \circ \eta(RA) = \id_{RA}, \quad
    \mathrm{ltr}_{\epsilon,\eta} : \prod_{A:\UUptd} \epsilon(LA) \circ L(\eta(A)) = \id_{LA}
  \end{displaymath}
\end{definition}
...
\begin{remark}
  As carefully proved in \cite[\githubpath theorems/homotopy/PtdAdjoint.agda]{hott-agda},
  any wild adjunction induces a dependent function $\Phi$ that maps pointed
  types $A,B:\UUptd$ to an equivalence
  \begin{displaymath}
  ...
  This dependent function is natural in the following sense: for any
  $A,B:\UUptd$, there are elements of $\Phi_{A,B}(\blank) \circ f = \Phi_{A',B}
  (\blank \circ L(f))$ for any $f:A'\ptdto A$ and $\Phi_{A,B'}(g\circ\blank) =
  R(g)\circ \Phi_{A,B} (\blank)$ for any $g:B\ptdto B'$.  \label{def:wild-adj}
\end{remark}
