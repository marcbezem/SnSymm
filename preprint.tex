...
\todo[inline]{TODO TODO TODO fix above and below!!}

\begin{lemma} \label{lem:forget-points-conn}
    Let $X$ be a type and $x : X$ a point. If $X$ is connected, then the canonical map which forgets the preservation of the base point $x$, given as
    \begin{alignat}{2}
...
\cref{prop:general-case-conn-comp-equiv} means that we can restrict ourselves to the connected component of $\id$ in $(\Sn n = \Sn n)$.
We next calculate the fundamental group of this type.





Recall that, in \cref{sec:circle-case}, we have seen that
\begin{equation}
(\Sn 1 = \Sn 1) = (\Sn 1 + \Sn 1).
\end{equation}
We show
...
Finally, we want to examine the structure of 
