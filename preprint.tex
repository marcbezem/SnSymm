...
\begin{proof}
  ...
  \end{align*}
  However, {\color{red}it follows from (\ref{eq:triangle-identity-unit})??} 
  that $\Phi_{\Sc,\Sp}(-\id_\Sp) = \inv{\eta_\Sc(\blank)}$ has an element. 
  Because $\loopspace\null(\inv\blank) = \inv\blank$.

({\color{red}abstract non sense from mustache style lemma, we
...
\begin{proof}
  ...
  possible. MB: isn't it clear enough?})
\end{proof}
...
All along this paper, we deal with pointed functions of the form $f:A \ptdto
...
we need a way to refer to these elemetary steps of path algebra, we give names
and conventions in this section.

Given a path $p:x=y$ in any type $A$, one should denotes
\begin{align*}
  \varpi_p &: \refl x = \inv p \refl y p\\
  \omega_p &: \refl x = \inv p p
\end{align*}
Depending on the implemetation of concatenation of path, $\omega_p$ and $\varpi_p$
...
\end{displaymath}
For any pair of composable path $p: x=y$ and $q: y=z$, one denotes
\begin{displaymath}
...
\end{displaymath}
As for the previous denoted path, we are mostly interested in the fact that 
\begin{displaymath}
...
Thze types $\UUptd$ does certainly not form a category, but some
