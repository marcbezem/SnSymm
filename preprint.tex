...
\begin{proof}
  ...
  \begin{equation}
    \begin{aligned}
      ...
      \ap{\bar f}(\glue(a,b)) &= f(a)(b) \quad \text{for }a:A,b:B 
    \end{aligned}
  ...
  are both pointed, for any $a:A$ and $b:B$, one has a an element $\ell_{a,b}$
  of $\loopspace{}(A*B)$ constructed as the following composition of paths:
  \begin{equation}
    \begin{tikzcd}[column sep=large]
      \inl(a_0) \rar["{\glue(a_0,b)}"] & \inr(b) \rar["\inv{\glue(a,b)}"] 
      & \inl(a) \rar["{\glue(a,b_0)}"] & \inr(b_0) \rar["\inv{\glue(a_0,b_0)}"]
      & \inl(a_0)
    \end{tikzcd}
    \label{eq:def-ellab-as-square}
  \end{equation}
  Then, to any $g:A*B \ptdto X$, one can map the function $\hat g: a \mapsto
  (b\mapsto \loopspace\null(g)(\ell_{a,b}))$. The map $\hat g$ is a pointed, as
  $\ell_{a_0,b_0} = \refl{\inl(a_0)}$ by path algebra. The construction $g
...
denote $a_0$ and $b_0$ the respective distinguished point of $A$ and $B$. 
...
then $A * B \weq \Sn{p+q+1}$\todo{Reference},
...
$\beta_0:\beta(N) = x_0$, and consider the evaluation fiber sequence
\[
...
%
In order to do so, we will reason on the map $\delta_\beta: (\susp \Sn n \ptdto X)
...
\beta(N) = \alpha(N)$ for short, where $\alpha_0:\alpha(N) = x_0$ is the path
pointing $\alpha$.
The map $\rho_\beta(\alpha)$ is pointed by the path $\beta_0 :
\rho_\beta(\alpha) (\inl a) \jdeq \beta(N) = x_0$.
...
\begin{equation}
  \begin{aligned}
    \gamma \times \gamma' (N) &\defequi \gamma(N) \\
    \gamma \times \gamma' (S) &\defequi \gamma'(S) \\
    \ap{\gamma \times \gamma'} (\mrd (b) ) &= 
...
Let us now define $\varphi$ from diagram \eqref{eq:Whitehead-general}. Since
$A$ is connected, hence the inclusion of $A \ptdto \conncomp{(\susp B \ptdto
X)}{\wunit}$ in $A \ptdto (\susp B \ptdto X)$ is an equivalence. Now, use the
equivalence between $\susp B \ptdto X$ and $B \to \loopspace\null X$ before
simply applying \cref{lemma:UMP-join-vs-loopspace}. Then the
...
\varphi(h) (\inl a) &\jdeq h(a)(N) \\
  \varphi(h) (\inr b) &\jdeq x_0 \\
  \ap{\varphi(h)} (\glue(a,b)) &= (h(a))_0 \cdot \ap{h(a)}(\sigma_B(b))
\end{align}
The map $\varphi(h)$ is pointed by the path $(h(a_0))_0 : h(a_0)(N) = x_0$.
...
\[
  ...
  : \beta(N) = \ast_X
\]
The map $\delta_\beta(\alpha)$ is itself pointed by the evident pointed homotopy
$\delta_\beta(\alpha)_\ast : \delta_\beta(\alpha)(\ast_A) \sim_\ast \beta$
that obtains from the identification $\sigma_A\,\ast_A = \refl{N}$.

With the preliminaries out of the way, let us show that $\rho_\beta$ can be identified with $\psi_\beta \defequi \varphi \circ (\xi_\beta \circ \blank) \circ \delta_\beta$. Let $\alpha : \susp A \ptdto X$.
...
\begin{align*}
  ...
  \ap{h\,a}(\mrd b)
         &= \ap{\beta}(\sigma_B\,b)^{-1}
           \cdot \beta_\ast^{-1}
           \cdot (\delta_\beta(\alpha)(a))_\ast
