...
\begin{displaymath}
  \inv p\cdot (\ap f\circ (\inv{\mrd(a)}\mrd(\blank)))\cdot p : A \to \loopspace\null B
\end{displaymath}
which is pointed by the path $\varpi_{f}:\refl b = \inv p \cdot (\ap f (\inv
\mrd(a)\mrd(a))) \cdot p$ given by path algebra operations.
...
\end{displaymath}
We now define $\bar d$ as the composition of $\loopspace\null\tau\circ
\gamma:(\Sp \ptdto\Sp) \to \loopspace\null\Sc$ with the well know isomorphism
$\loopspace \null \Sc \weq \ZZ$.
