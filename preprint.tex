...
\begin{center}
  \begin{tikzcd}[column sep=6em,labels={font=\normalsize}]
    m_N^{-1}\cdot {\color{darkgreen}m_N} 
      \arrow[swap, equal, "\downarrow"']{dd}{\ap{(\blank\cdot {\color{darkgreen}m_N})\circ\inv{}}(\beta)}&&
    {\color{blue}\inv{m_S}}\cdot m_S 
      \arrow[swap, equal, "{c(m_S,m_N)}", "\leftarrow"']{ll}
      \arrow[equal, "\uparrow"']{dd}{\ap{{\color{blue}\inv{m_S}}\cdot\blank}(\beta)}
 \\\\
      \inv{m_S}\cdot {\color{darkgreen}m_N} &&
      {\color{blue}\inv{m_S}}\cdot m_N \arrow[swap, equal, "\leftarrow"']{ll}{\refl{}}
  \end{tikzcd}
\end{center}
...
%	\end{equation}
       ...
       the easiest way to show that the above diagram commutes
       ...
       But now, as $m_N \jdeq \refl N$, all paths appearing in the above diagram,
...
\begin{theorem}[Conjecture + start of proof?]
  $\fgr (\Sp = \Sp) \neq 0$
...
\begin{proof}
  ...
  \begin{displaymath}
    \dots \to \hgr 2 (\Sp) \overset d \to \fgr (\Sp \ptdto \Sp) \to \fgr (\Sp \to \Sp)
    \to \fgr (\Sp) \to \dots
  \end{displaymath}
  As $\fgr (\Sp) = 0$, the map $\fgr(\Sp \ptdto \Sp) \to \fgr (\Sp \to \Sp)$ is
  an epimorphism. In order to prove that $\fgr(\Sp \to \Sp) \neq 0$, it
  suffices to show that the kernel of this map is not the whole group $\fgr(\Sp
  \ptdto \Sp)$. However, this kernel is precisely the image of the map denoted
  $d$ in virtue of the exactness of the sequence. Hence, we are to prove that
  $d$ is not surjective.

  Let us try to understand $d$: by definition of the long exact sequence
  (cf.~\cite[Thm.~8.4.6]{HoTT}), $d$ is the $\setTrunc {\loopspace{} (\partial)
  \circ \inv{\blank}}$ where $\partial : \loopspace{} (\Sp) \to (\Sp \ptdto
  \Sp)$ is defined by
  \begin{equation}
    p \mapsto (\id_{\Sp}, p)
    \label{eq:def-delta}
