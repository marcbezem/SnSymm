...
\begin{center}
  \begin{tikzpicture}
    \matrix (m) [matrix of nodes, column sep=4em, row sep=5em] {
      N & S & N  & & N & S & N \\
      N &   &    & &   &   & N \\
    };%
    ...
    \draw[->] (m-1-3.north) to[bend left=60] node[above, name=Bfrom] {$m_S;\inv{m_S}$} (m-1-5.north);%
    \draw[->] (m-1-3.south) to[bend right=60] node[below, name=Bto] {$m_N;\inv{m_N}$} (m-1-5.south);%
