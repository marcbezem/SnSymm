...
\begin{proof}
	...
	\draw[-latex,text=darkgreen] (N1) to node[left] {$m_N$} (S1);
	\draw[-latex,text=darkblue] (N2) to node[right] {$m_S$} (S2);
	\draw[-latex] (N1) to node[above] {$m_S$} (S2);
	\draw[-latex] (N2) to node[above] {$m_N$} (S1);
	\end{tikzpicture}
	\end{equation}
	with the obvious commutativity proof $p$.
%	 : m_N \cdot \inv{m_N} = m_S \cdot \inv{m_S}$.
	...
	\end{tikzpicture}
	\end{equation}
	with the obvious commutativity proof $q$.
	...
	In this case, we need to show an equality between the discussed squares \eqref{eq:N-commute} and \eqref{eq:S-commute}, and this equality depends on (i.e.\ lies over) the equality that stems from the top horizontal arrow in \eqref{eq:y-commute}, i.e.\  $\ap{\mrd_{\susp(\susp(A))}}\left(\ap{-\id_{\susp(\susp(A))}}(\mrd_{\susp(A)}(z))\right)$,
	and the equality stemming from the bottom horizontal arrow in \eqref{eq:y-commute}, i.e.\
	$\ap{\mrd_{\susp(\susp(A))}}(\mrd_{\susp(A)}(z)))$.
...
\begin{proof}
	...
	Nicolai: This should now follow from the new subsection above. I'm working on it.}
\end{proof}
...
%%
    ...
    \item Building on the previous examples and
      ...
      by successive transport and concatenation of the same witnesses for
      $\loopspace\null$ and $\setTrunc \blank$.
%%      \todo{Should it be made more precise by defining composition of wild functors in full gen. ?}
  \end{enumerate}
\end{example}
