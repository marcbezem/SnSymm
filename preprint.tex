...
\begin{proof}
  ...
  \end{alignat}
	...
	After rewriting path-overs as path composition using \cite[Lem~2.11.3]{HoTT}, the type of this component is equivalen to the the type of commutativity proof for the square:
	...
	\draw[-latex] (N1) to node[above] {$\mrd(-\id_{\susp \susp A}(y))$} (S2);
...
%	 : m_N \cdot \inv{m_N} = m_S \cdot \inv{m_S}$.
	...
	Finally, we consider the third case of the induction on $y$, namely the case that $y$ is a path of the form $\mrd(z)$ for some $z : A$.
        ...
        \begin{displaymath}
          U(y) \defequi \left( \inv{m_S} \cdot \mrd(-\id_{\susp\susp A}(y)) = \inv{\mrd(y)} \cdot m_N\right)
        \end{displaymath}
...
%%      (i.e.\ depends on) the two equalities that stem from the top horizontal arrow and the bottom horizontal arrow in \eqref{eq:y-commute} (note that the two vertical arrows in \cref{eq:y-commute} do not depend on $y$).
        ...
        \begin{displaymath}
          ...
          \cdot c \cdot
          \inv{\left( \ap{\inv{m_S} \cdot \blank} (\ap {\mrd \circ -\id_{\susp\susp A}} (p)) \right)}
          : U(y')
        \end{displaymath}
        ...
        \begin{alignat}{2}
	\alpha &\defequi \ap{\mrd}\left(\ap{-\id_{\susp\susp A}}(\mrd(z))\right) && : m_S=m_N
        \label{eq:alpha-def}
        \\
	...
	\end{equation}
        ...
        Unfolding the definition of $-\id_{\susp\susp A}$ in~\eqref{eq:alpha-def} and using standard path algebra steps, we get $\alpha = \inv{\beta}$, and then \eqref{eq:first-square-composed} simplifies to
...
\begin{remark} Experimental picture
\begin{center}
  \begin{tikzpicture}
    \matrix (m) [matrix of nodes, column sep=4em, row sep=9em] {
      N & S & N  & & N & S & N \\
      N &   &    & &   &   & N \\
    };%
    \draw[->] (m-1-1.north) to[bend left=80] node[above, name=Afrom] {$m_N$} (m-1-2.north);%
    \draw[->] (m-1-1.south) to[bend right=80] node[below, name=Ato] {$m_S$} (m-1-2.south);%
    \draw[cell] (Afrom) to node[fill=white] {$\inv{\alpha}$} (Ato);%

    \draw[->] (m-1-2) to node[above] {$\inv{m_S}$} (m-1-3);%

    \path[->] (m-1-3.north) to[bend left=60] node[name=Bfrom] {$S$} (m-1-5.north);%
    \draw[->] (m-1-3.north) to[bend left, out=60] node[above] {$m_S$} (Bfrom.west);
    \draw[->] (Bfrom.east) to[bend left, in=120] node[above] {$\inv{m_S}$} (m-1-5.north);
    
    
    \path[->] (m-1-3.south) to[bend right=60] node[name=Bto] {$S$} (m-1-5.south);%
    \draw[->] (m-1-3.south) to[bend right, out=-60] node[below] {$m_N$} (Bto.west);
    \draw[->] (Bto.east) to[bend right, in=-120] node[right] {$\inv{m_N}$} (m-1-5.south);
    \draw[cell] (Bfrom) to node[fill=white] {$c(m_S,m_N)$} (Bto);%

    \draw[->] (m-1-5) to node[above] {$m_N$} (m-1-6);%

    \draw[->] (m-1-6.north) to[bend left=80] node[above, name=Cfrom] {$\inv{m_N}$} (m-1-7.north);%
    \draw[->] (m-1-6.south) to[bend right=80] node[below, name=Cto] {$\inv{m_S}$} (m-1-7.south);%
    \draw[cell] (Cfrom) to node[fill=white] {$\ap{\inv{}}(\beta)$} (Cto);%

    \draw[->] (m-2-1) to node[below] {$m_N;\inv{m_S}$} (m-2-7);%
   
    \draw[->, shorten >= 2pt] (m-1-1.south) to[bend right, out=-90, in=-90] node[below] {$\refl N$} (m-1-3.south);
    \draw[->, shorten >= 2pt] (Bto.south) to[bend right, out=-90, in=-120] node[below] {$\refl S$} (m-1-6.south);
    \draw[->, shorten >= 2pt] (m-1-2.north) to[bend left, out=60, in=90] node[above] {$\refl S$} (Bfrom.north);
    \draw[->, shorten >= 2pt] (m-1-5.north) to[bend left, out=90, in=90, looseness=1.3] node[above] {$\refl N$} (m-1-7.north);
    \draw[->, shorten >= 2pt] (m-1-1.north) to[bend left, out=90, in=90, looseness=1.3] node[above] {$m_N$} (Bfrom.north);
    \draw[->, shorten >= 2pt] (Bfrom.north) to[bend left, out=90, in=90, looseness=1.3] node[above] {$\inv{m_S}$} (m-1-7.north);
    \draw[->, shorten >= 2pt] (m-1-1.south) to[bend right, out=-90, in=-90, looseness=1.3] node[below] {$m_N$} (Bto.south);
    \draw[->, shorten >= 2pt] (Bto.south) to[bend right, out=-90, in=-90, looseness=1.3] node[below] {$\inv{m_S}$} (m-1-7.south);
  \end{tikzpicture}
\end{center}
\end{remark}
