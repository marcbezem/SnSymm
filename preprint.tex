...
\end{equation}
The map $\gamma \times \gamma'$ is pointed by the path $\gamma_0: \gamma(N) =
...
$\alpha_0\cdot \ap\alpha(\sigma_A(a_0)) \cdot \miniast \alpha \beta$; using
that the path $\sigma_A(a_0) = \refl N$ given by path algebra, one finds a path
...
g(\inl a) &\jdeq h(a)(N) = \beta(N) \\
  g(\inr b) &\jdeq x_0 \\
  \ap{g}(\glue(a,b))
  &= (h(a))_0 \cdot \ap{h(a)}(\sigma_B(b)) \\
  &= (h(a))_0 \cdot \ap{h(a)}(\inv{\mrd(b_0)} \cdot\mrd b) \\
  &= (h(a))_0 \cdot \inv{\ap{h(a)}(\mrd{\ast_B})}
    \cdot \ap{h(a)}(\mrd (b)) \\
    &= \bigl(\alpha_0 \cdot \ap{\alpha}(\sigma_A(a))
           ...
           \cdot \ap{\beta}(\sigma_B\,b)\bigr) \\
            &= \beta_0
              ...
              \cdot \ap{\beta}(\sigma_B\,b)
\end{align*}
The path $g_0$ pointing $g$ is, by definition of $\varphi$, the path
$(h(a_0))_0 = \alpha_0 \cdot \ap\alpha(\sigma_A(a_0)) \cdot \miniast \alpha
\beta$. It only remains to construct a path $H : \rho_\beta(\alpha) = g$ as
mere functions such that the type $g_0 \cdot H(\inl(a_0)) = \beta_0$ as an
element. We proceed by induction on an element of the join:
\begin{align*}
  H(\inl a) &\defequi \refl{\beta(N)} : \beta(N) = \beta(N) \\
  H(\inr b) &\defequi \beta_0 \cdot \ap{\beta}(\sigma_B(b))^{-1} \cdot
              \miniast\beta\alpha : \alpha(N) = x_0
\end{align*}
...
\[
  \begin{tikzcd}
    \beta(N)  \ar[ddd,"{\refl{\beta(N)}}"']\ar[r,"{\ap{\beta}(\sigma_B\,b)}"] &[10pt]
    \beta(N)  \ar[r,"{\miniast\alpha\beta}"] &[-15pt]
    \alpha(N) \ar[r,"{\ap{\alpha}(\sigma_A\,a)}"] &[10pt]
    \alpha(N) \ar[rrr,"{\refl{\alpha(N)}}"] &[-15pt]
    &[10pt]
    &[-20pt]
...
%\ast_X
  ...
  \end{tikzcd}
\]
We fill this with standard path algebra operations. This proves that
$\rho_\beta(\alpha)$ and $g$ are equal as mere functions. We must still check
that $g_0 \cdot H(\inl(a_0)) = \beta_0$. As $H(\inl(a_0)) = \refl {\beta(N)}$,
one can rely on path algebra operation to reduce $(h_(a_0))_0$ to $\beta_0$.
