...
%% biblatex for @online class
\usepackage[
  backend=biber,
  bibencoding=utf8,
style=alphabetic,maxbibnames=9,maxcitenames=9]
  {biblatex}
...
\author{Pierre Cagne\thanks{Universitetet i Bergen} \and Nicolai
  Kraus\thanks{University of Nottingham} \and Marc
...
\begin{abstract}
  ...
  The case $n=1$ of the circle has a slick answer: $(\Sc = \Sc) \weq (\Sc +
  \Sc)$. Unfortunately, the generalisation of this result to $n>1$ is provably wrong.
  Instead, we show that the type $\Sn n = \Sn n$ consists of
  two equivalent connected components. We construct concrete elements
  in each component.
\end{abstract}
...
%%\footnote{\color{red}Or
%%  maybe we should make an appendix of that? MB is for an appendix!}
:
\begin{enumerate}
...
%%  \Phi_{A,B}:
%%\end{displaymath}
%%that takes a function $f:\susp A \to B$ pointed by $f_0: f(N)=b$ and maps it to 
%%\begin{displaymath}
%%  {f_0}\cdot (\ap f\circ (\inv{\mrd(a)}\mrd(\blank)))\cdot \inv{f_0} : A \to \loopspace\null B
%%\end{displaymath},
%%which is pointed by path algebra.
%%
%%One has morevover, for each type pointed type $A$, a {\em unit} function 
%%\begin{displaymath}
...
%%  \label{eq:triangle-identity-unit}
%%\end{equation}
%%given by simple path algebra: for any map $f:\susp A \to B$ pointed by $f_0:f(N)=b$
%%\begin{displaymath}
...
\begin{proof}
  ...
  \begin{displaymath}
    \begin{tikzcd}
      ...
      & (\hgr 2(\Sp) \to \hgr 2(\Sp)) \drar["\ev_{\inv\zeta(1)}"] \ar[dd,"\circled{5}", phantom] &
      \\
...
\subsection{The Connected Components of \texorpdfstring{$\boldsymbol{\Sn n = \Sn n}$}{Sn = Sn}}
...
\begin{corollary}[{of \cref{thm:susp-is-monoid-iso}}] \label{cor:iso-of-monoids}
...
%\begin{corollary} \label{cor:forget-point-iso}
...
in \cite{doublesusp}. Here, we give the argument in the style of \cite{HoTT}:
\begin{proof}
  ...
  \end{alignat}
	...
	After rewriting path-overs as path composition using \cite[Lem~2.11.3]{HoTT}, the type of this component is equivalent to the the type of commutativity proofs for the square:
	...
	\draw[->,text=darkgreen] (N1) to node[left] {$\mrd(N_{\susp A})$} (S1);
        \draw[->,text=darkblue] (S2) to node[right] {$\inv{\mrd(S_{\susp A})}$} (N2);
...
%%      (i.e.\ depends on) the two equalities that stem from the top horizontal arrow and the bottom horizontal arrow in \eqref{eq:y-commute} (note that the two vertical arrows in \cref{eq:y-commute} do not depend on $y$).
 ...
 %\todo[inline]{One can use whiskering here if you find it easier to parse.}
	...
	\end{tikzpicture}
	\end{equation}
        The goal is now to show that \eqref{eq:first-square-composed} is equal 
        to the trivial proof $\refl{}$ filling square~\eqref{eq:S-commute},
        the outer `square' of (\ref{eq:N-commute-with-ears}).
        Unfolding the definition of $-\id_{\susp A}$ in~\eqref{eq:alpha-def} and using standard path
        algebra steps, we get $\alpha = \inv{\beta}$, 
...
%	\end{equation}
       ...
       As suggested by Ulrik Buchholtz \cite{doublesusp},
       the easiest way to show that (\ref{eq:3-cell}) commutes
       is to generalise to arbitrary points $N$, $S$ and arbitrary paths $m_N,m_S : N=S$, 
       as well as to an arbitrary higher path $\beta: m_N=m_S$. One starts by doing path 
       induction on $m_N$, reducing the task to the case $S\jdeq N$ and $m_N\jdeq \refl N$,
       and arbitrary $m_S:N=N$ and $\beta: m_N = m_S$. One can then do path induction on $\beta$, 
       reducing the task to the case $m_S\jdeq m_N$ and $\beta \jdeq \refl{m_N}$. 
...
\begin{corollary}\label{cor:id-not-minus-id}
...
map to $D$.  If the induced map is an equivalence, then we say the
square is a \emph{pushout square}.

The other higher inductive types we've used so far, the circle,
suspensions, and truncations, can all be defined in terms of
pushouts. For example, the suspension $\susp A$ is the pushout of the unique
span $1 \leftarrow A \to 1$.

%Another instance of a pushout if the \emph{cofiber} of a map $f : A \to B$,
%$\cof(f)$:
%\[
%  \begin{tikzcd}
%    A \ar[r,"f"]\ar[d]\ar[pushout] & B \ar[d,"\inr"] \\
%    1 \ar[r,"\inl"'] & \cof(f)
%  \end{tikzcd}
%\]
Recall that the \emph{join} $A * B$ of two types $A$ and $B$
is the pushout of the product projections:
\[
  \begin{tikzcd}
  ...
  \end{tikzcd}
\]
Also, the \emph{wedge sum} $A \vee B$ of pointed types $A$ and $B$
is the pushout of the point inclusions:
\[
  \begin{tikzcd}
    1 \ar[r,"\ast_B"]\ar[d,"\ast_A"']\ar[pushout] & B \ar[d,"\inr"] \\
    A \ar[r,"\inl"'] & A \vee B
  \end{tikzcd}
\]
In this case, we have the \emph{wedge inclusion} $i : A \vee B \to A \times B$
with $i(\inl a) \jdeq (a , \ast_B)$, $i(\inr b) \jdeq (\ast_A, b)$,
and $\ap{i}(\glue\ast) = \refl{(\ast_A,\ast_B)}$.

For the rest of this section we fix two pointed types $A$ and $B$.
...
\begin{definition}
  ...
  \]
\end{definition}
%Recall that the \emph{smash product} $A \wedge B$ of pointed types $A$ and $B$
%is the cofiber of the wedge inclusion,
%\[
%  \begin{tikzcd}
%    A \vee B \ar[r,"i"]\ar[d]\ar[pushout] & A \times B \ar[d,"q"] \\
%    1 \ar[r,"*"'] & D,
%  \end{tikzcd}
%\]
%where we write $q : A \times B \to A \wedge B$ for the quotient map.
%\begin{lemma}
%  The map $\theta : A * B \to \susp(A \times B)$
%  defined by $\theta(\inl a) \jdeq N$,
%  $\theta(\inr b) \jdeq S$, and
%  $\ap\theta(\glue(a,b)) = \mrd(a,b)$,
%  induces an equivalence
%  $\susp q \circ \theta : A * B \to \susp(A \wedge B)$.
%\end{lemma}
%In terms of this, we can give an alternate definition of the Whitehead product map.
%\begin{definition}
%  Let $W' = W'_{A,B} : \susp(A \wedge B) \to \susp A \vee \susp B$
%  be induced from the commutator of the projections,
%  $[p_1,p_2] = p_1 + p_2 - p_1 - p_2 : \susp(A \times B) \to \susp A \vee \susp B$,
%  together with the nullhomotopy of the composition with the wedge inclusion,
%  $[p_1,p_2] \circ \susp i : \susp(A \vee B) \to \susp A \vee \susp B$.
%\end{definition}
%Here we're using the identification $\susp(A \wedge B) = \susp(\cof(i))
%= \cof(\susp i)$.
%In the case where $A \jdeq B \jdeq \Sn 0$, this reproduces the usual attaching map for the torus, $\Sc \to \Sc \vee \Sc$.
%Note that there's in general no projection maps from a smash product,
%$A \wedge B \to A$ or $A \wedge B \to B$.
%\begin{lemma}
%  There is an identification $W = W' \circ \susp q \circ \theta
%  : A * B \to \susp A \vee \susp B$.
%\end{lemma}
%\begin{proof}
%  By definition of $W'$ we have for $k \defequi W' \circ \susp q \circ \theta$
%  that $k(\inl a) \jdeq \inl N$, $k(\inr b) \jdeq \inr N$, and
%  \begin{gather*}
%    \ap{k}(\glue(a,b)) = \ap{\inr}(\sigma_B\,b)^{-1} \cdot \glue\ast
%    \cdot \ap{\inl}(\sigma_A\,a)^{-1} \cdot (\glue\ast)^{-1} \\
%    \cdot \ap{\inr}(\sigma_B\,b) \cdot \glue\ast
%    \cdot \ap{\inl}(\sigma_A\,a).
%  \end{gather*}
%  We define a homotopy $h$ from $W$ to $k$
%  using induction on the join.
%  We set:
%  \begin{align*}
%    h(\inl a) &\defequi \ap{\inl}(\sigma_A\,a)^{-1} \cdot (\glue\ast)^{-1}
%  : \inr N = \inl N \\
%    h(\inr b) &\defequi (\sigma_B\,b)^{-1} \cdot \glue\ast : \inl N = \inr N
%  \end{align*}
%  Finally, we must produce an element of
%  $\prod_{a\from A}\prod_{b\from B} \pathover {h(\inl a)} {\lambda z.W(z)=k(z)}
%  {\glue(a,b)} {h(\inr b)}$, which corresponds to filling the following square:
%  \[
%    \begin{tikzcd}
%      \inr N\ar[d,"{(\glue\ast)^{-1}}"']\ar[rr,"{\refl{}}"] &
%      &
%      \inr N\ar[r,"{\ap{\inr}(\sigma_B\,b)}"] &
%      \inr N\ar[r,"{(\glue\ast)^{-1}}"] &
%      \inl N\ar[r,"{\ap{\inl}(\sigma_A\,a)^{-1}}"] &
%      \inl N\ar[rr,"{\refl{}}"] &
%      &
%      \inl N\ar[d,"{\glue\ast}"]
%      \\
%      \inl N\ar[d,"{\ap{\inl}(\sigma_A\,a)^{-1}}"'] &
%      &
%      &
%      &
%      &
%      &
%      &
%      \inr N\ar[d,"{\ap{\inr}(\sigma_B\,b)^{-1}}"]
%      \\
%      \inl N\ar[r,"{\ap{\inl}(\sigma_A\,a)}"'] &
%      \inl N\ar[r,"{\glue\ast}"'] &
%      \inr N\ar[r,"{\ap{\inr}(\sigma_B\,b)}"'] &
%      \inr N\ar[r,"{(\glue\ast)^{-1}}"'] &
%      \inl N\ar[r,"{\ap{\inl}(\sigma_A\,a)^{-1}}"']&
%      \inl N\ar[r,"{\glue\ast}"'] &
%      \inr N\ar[r,"{\ap{\inr}(\sigma_B\,b)^{-1}}"'] &
%      \inr N
%    \end{tikzcd}
%  \]
%  We fill this with standard path algebra operations.
%\end{proof}
% By composing pushout squares, we get the following diagram:
% \[
%   \begin{tikzcd}
%     \susp(A \wedge B) \ar[r,"W'"]\ar[d]\ar[pushout]
%     & \susp A \vee \susp B\ar[d,"i"]\ar[r]\ar[pushout] & 1\ar[d] \\
%     1 \ar[r] & \susp A \times \susp B\ar[r] & \susp^2(A \wedge B)
%   \end{tikzcd}
% \]
We now also fix another pointed type $X$.
\begin{definition}
...
then $A * B \weq \Sn{p+q+1}$,
so we obtain a map
\[
  [\blank,\blank] : \hgr {p+1}(X) \times \hgr {q+1}(X) \to \hgr{p+q+1}(X),
\]
which is the usual Whitehead product on homotopy groups.

Fix now a pointed map $\beta : \susp B \ptdto X$,
and consider the evaluation fiber sequence
\[
...
Before taking set truncations, we identify both in the following diagram,
which we're going to show commutes:
\begin{equation}\label{eq:Whitehead-special}
  \begin{tikzcd}
    (\susp{\Sn n} \ptdto X)
    \ar[r,"\partial_\beta"] \ar[d,"\rho_\beta"'] &
    (\Sn n \ptdto \conncomp{(\susp B \ptdto X)}{\beta})
    \ar[d,"\xi_\beta\circ\blank","\sim" rotninety] \\
    (\Sn n * B \ptdto X) &
    (\Sn n \ptdto \conncomp{(\susp B \ptdto X)}{0}) \ar[l,"\sim"]
  \end{tikzcd}
\end{equation}
Since nothing hinges on having a sphere $\Sn n$, let us generalize
and construct a commuting diagram for any pointed type $A$:
...
explicitly:
\newcommand*\miniast[2]{\mathbin{{}_{#1}^{#2}\ast}}
\begin{align*}
  \rho_\beta(\alpha)\,(\inl a) &\jdeq \beta(N) \\
  \rho_\beta(\alpha)\,(\inr b) &\jdeq \alpha(N) \\
  \ap{\rho_\beta(\alpha)} (\glue(a,b)) &= \ap{\alpha}(\sigma_A\,a) \cdot
                                         \miniast\alpha\beta \cdot
                                         \ap{\beta}(\sigma_B\,b)
\end{align*}
Here we write $\miniast\alpha\beta \defequi \alpha_\ast^{-1} \cdot \beta_\ast : \beta(N) = \alpha(N)$ for short.
The map $\rho_\beta(\alpha)$ is pointed by $\beta_\ast : \beta(N) = \ast_X$.

Since $\susp B$ is a wild cogroup, the type $\susp B \ptdto X$
is a wild group, so all its components are equivalent.
Concretely, we have an equivalence
$\xi_\beta : \conncomp{(\susp B \ptdto X)}{\beta} \weq_\ast
\conncomp{(\susp B \ptdto X)}{0}$ given by
(we don't include the propositional parts, which are in any case not needed):
\begin{align*}
  \xi_\beta(\gamma)\, (N) &\jdeq \gamma(N) \\
  \xi_\beta(\gamma)\, (S) &\jdeq \beta(N) \\
  \ap{\xi_\beta(\gamma)}(\mrd b)
                          &= \ap{\beta}(\sigma_B\,b)^{-1} \cdot
                            \miniast\beta\gamma \cdot
                            \ap{\gamma}(\sigma_B\,b)
\end{align*}
The bottom map in \eqref{eq:Whitehead-general} corresponds to the wild adjunction
...
\to (A * B \ptdto X)$:
\begin{align*}
  \varphi(h) (\inl a) &\jdeq h(a,N) \\
  \varphi(h) (\inr b) &\jdeq \ast_X \\
  \ap{\varphi(h)} (\glue(a,b)) &= (h\,a)_\ast \cdot \ap{h\,a}(\sigma_B\,b)
\end{align*}
The map $\varphi(h)$ is pointed by the path $(h\,\ast_A)_\ast : h(\ast_A,N) = \ast_X$.

Before we can verify the commutativity of \eqref{eq:Whitehead-general},
we also need to spell out the action of the boundary map $\partial_\beta$ itself:
$\partial_\beta(\alpha)(a) \jdeq \beta$ as an unpointed function,
but with a new pointedness path:
\[
  (\partial_\beta(\alpha)(a))_\ast \defequi
  \loopspace\null\alpha(\sigma_A\,a) \cdot \beta_\ast
  = \alpha_\ast \cdot \ap{\alpha}(\sigma_A\,a) \cdot \miniast\alpha\beta
  : \beta(N) = \ast_X
\]
The map $\partial_\beta(\alpha)$ is itself pointed by the evident pointed homotopy
$\partial_\beta(\alpha)_\ast : \partial_\beta(\alpha)(\ast_A) \sim_\ast \beta$
that obtains from the identification $\sigma_A\,\ast_A = \refl{N}$.

With the preliminaries out of the way, let us show that $\rho_\beta$ can be identified with $\psi_\beta \defequi \varphi \circ (\xi_\beta \circ \blank) \circ \partial_\beta$. Let $\alpha : \susp A \ptdto X$.
Unfolding the above definitions, let us first examine
$h \defequi \xi_\beta\circ (\partial_\beta(\alpha)) : A \ptdto \conncomp{(\susp B\ptdto X)}{0}$:
\begin{align*}
  h(a,N) &\jdeq h(a,S) \jdeq \beta(N) \\
  \ap{h\,a}(\mrd b)
         &= \ap{\beta}(\sigma_B\,b)^{-1}
           \cdot \beta_\ast^{-1}
           \cdot (\partial_\beta(\alpha)(a))_\ast
         ...
         &= \ap{\beta}(\sigma_B\,b)^{-1}
           ...
           \cdot \ap{\beta}(\sigma_B\,b) \\
  (h\,a)_\ast
         &= (\partial_\beta(\alpha)(a))_\ast \\
         &= \alpha_\ast \cdot \ap{\alpha}(\sigma_A\,a)
           \cdot \miniast\alpha\beta
\end{align*}
...
\begin{align*}
  g(\inl a) &\jdeq h(a,N) = \beta(N) \\
  g(\inr b) &\jdeq \ast_X \\
  \ap{g}(\glue(a,b))
            &= (h\,a)_\ast \cdot \ap{h\,a}(\sigma_B\,b) \\
            &= (h\,a)_\ast \cdot \ap{h\,a}(\mrd{\ast_B}^{-1}\cdot\mrd b) \\
            &= (h\,a)_\ast \cdot \ap{h\,a}(\mrd{\ast_B})^{-1}
              \cdot \ap{h\,a}(\mrd b) \\
            &= \bigl(\alpha_\ast \cdot \ap{\alpha}(\sigma_A\,a)
           ...
           \cdot \ap{\beta}(\sigma_B\,b)\bigr) \\
            &= \beta_\ast
              ...
              \cdot \ap{\beta}(\sigma_B\,b)
\end{align*}
We also get $g_\ast = \beta_\ast$. It only remains to construct a pointed homotopy $H : \rho_\beta(\alpha) \sim_\ast g$. Again this is done by induction on an element of the join:
\begin{align*}
  H(\inl a) &\defequi \refl{} : \beta(N) = \beta(N) \\
  H(\inr b) &\defequi \beta_\ast \cdot \ap{\beta}(\sigma_B\,b)^{-1} \cdot
              \miniast\beta\alpha : \alpha(N) = \beta(N)
\end{align*}
Finally, we must produce an element of
$\prod_{a\from A}\prod_{b\from B} \pathover
{H(\inl a)} {\lambda z.\rho_\beta(\alpha)(z)=g(z)} {\glue(a,b)} {H(\inr b)}$,
which corresponds to filling the following square:
\[
  \begin{tikzcd}
    \beta(N)  \ar[ddd,"{\refl{}}"']\ar[r,"{\ap{\inr}(\sigma_B\,b)}"] &[10pt]
    \beta(N)  \ar[r,"{\miniast\alpha\beta}"] &[-15pt]
    \alpha(N) \ar[r,"{\ap{\inl}(\sigma_A\,a)}"] &[10pt]
    \alpha(N) \ar[rrr,"{\refl{}}"] &[-15pt]
    &[10pt]
    &[-20pt]
    \alpha(N) \ar[d,"{\miniast\beta\alpha}"]
    \\[-5pt]
    &
    &
    &
    ...
    &
    &
    \beta(N)  \ar[d,"{\ap{\inr}(\sigma_B\,b)^{-1}}"]
    \\[10pt]
    &
    &
    &
    ...
    &
    &
    \beta(N)  \ar[d,"{\beta_\ast}"]
    \\[-10pt]
    \beta(N)  \ar[r,"{\ap{\inr}(\sigma_B\,b)}"'] &
    \beta(N)  \ar[r,"{\miniast\alpha\beta}"'] &
    \alpha(N) \ar[r,"{\ap{\inl}(\sigma_A\,a)}"'] &
    \alpha(N) \ar[r,"{\miniast\beta\alpha}"'] &
    \beta(N)  \ar[r,"{\ap{\inr}(\sigma_B\,b)^{-1}}"'] &
    \beta(N)  \ar[r,"{\beta_\ast}"'] &
    \ast_X
  \end{tikzcd}
\]
We fill this with standard path algebra operations.
...
\begin{theorem}\label{thm:sort-of-ehp}
...
\begin{proof}
  ...
  induction that $\phi_{\Sn n}^n$ maps $(\id_{\Sn n}, \refl N)$ to a point in
  ...
  \begin{equation}
    \psi_n (\alpha) \defequi \inv{\phi_{\Sn n}^n(\id_{\Sn n},\refl N)}\cdot \alpha
...
\end{theorem}
\begin{proof}  (cf.~\cite{gwwhitehead})
  ...
  generator, namely $i_2 \defequi \settrunc{\phi_{\Sp}^2(\id_\Sp,\refl N)}$ (we
  ...
  \begin{equation}
    ...
    \label{eq:def-delta}
  \end{equation}
  We can then compute $\xi(\delta(i_2))$ a little further:
  \begin{align*}
    \xi(\delta(i_2)) &= \fgr(\psi_2\circ\phi_\Sp^2)
    \left(
      \settrunc{\loopspace{}(\partial)(\inv{i_2})}
    \right) 
    \\
    &= \settrunc{\loopspace{}(\psi_2 \circ \phi_\Sp^2 \circ \partial)(\inv{i_2})} 
  \end{align*}
  However, the map $\psi_2\circ \phi_\Sp^2\circ \partial$ maps an element
  $p:\loopspace{}(\Sp)$ to the element $\inv{\phi_\Sp^2(\id_\Sp,\refl N)}\cdot
  \phi_\Sp^2(\id_\Sp, p)$. Unfolding the definition of $\phi_\Sp^2$, we find: 
  \begin{equation}
    \psi_2 \circ \phi_\Sp^2\circ \partial (p) = 
    \inv{\loopspace{}(\sigma_\Sc)(\Sloop)} \cdot 
    \loopspace{} (p\cdot \sigma_\Sc(\blank) \cdot \inv p) (\Sloop)
    \label{eq:unfolding-delooping-xi-delta}
  \end{equation}
%%
%%  In particular, path induction shows that $\loopspace{}(\partial)$ maps a loop
%%  $\alpha : \refl N = \refl N$ to the element of $(\id_\Sp,\refl N) = (\id_\Sp,
%%  \refl N)$ whose components are:
%%  \begin{equation}
%%    \refl{\id_\Sp} : \id_\Sp = \id_\Sp, \quad \alpha: \refl N = \refl N
%%  \end{equation}
%%  where the second component is seen as the filler for the triangle:
%%  \begin{equation}
%%    \begin{tikzcd}
%%      & N \dlar["\refl N"swap] \drar["\refl N"] &
%%      \\
%%      \id_\Sp(N) \ar[rr,"\refl{\id_\Sp}(N)" swap] & & \id_\Sp(N)
%%    \end{tikzcd}
%%  \end{equation}
%%  Moreover, by the proof of \cref{lem:fgr-ptd-endomaps-Sn}, $\xi$ is the map
%%  Recall that $\hgr 2 (\Sp) \simeq \ZZ$ and that this equivalence identifies $1:\ZZ$ with 
%%  \begin{equation}
%%    \upsilon \defequi \settrunc{\loopspace{}(\sigma_{\Sc})(\Sloop)} : \hgr 2(\Sp)
%%    \label{eq:1-in-pi2-S2}
%%  \end{equation}
%%  We can compute $d(\upsilon)$ further:
%%  \begin{equation}
%%    d(\upsilon) = d(\settrunc{\loopspace{}(\sigma_\Sc)(\Sloop)})
%%    = \settrunc{\loopspace{}(\partial\circ \sigma_\Sc)(\inv \Sloop)}
%%  \end{equation}
%%  We want to understand the subgroup generated by $d(\upsilon)$ in $\fgr (\Sp \ptdto\Sp)$
%%  
%%  To prove that $d$ is not surjective, one need only to prove that
%%  $d(\upsilon)$ does not generates $\fgr(\Sp \ptdto \Sp)$. That is one only
%%  needs to find an element $\alpha:(\id_\Sp,\refl N) = (\id_\Sp, \refl N)$ such
%%  that for all $k:\ZZ$, $\settrunc\alpha \neq d(\upsilon)^k$. Notice that for
%%  $k:\ZZ$, 
%%  \begin{equation}
%%    d(\upsilon)^k = d(\settrunc{\loopspace{}(\sigma_\Sc)(\Sloop^k)})
%%    = \settrunc{\loopspace{}(\partial)\inv{ \left( 
%%        \loopspace{}(\sigma_\Sc)(\Sloop^k)
%%    \right)}}
%%  \end{equation}
%%  \todo[inline]{Find such an $\alpha$! I'm not sure how yet.}
\end{proof}

This calculation also allows us to see that the result from
\cref{sec:circle-case} does not generalise:
