...
\begin{proof}
  ...
  set, hence as $n\geq 2$ this is an $(n-2)$-type. The sphere $\Sn n$ being
  $(n-1)$-connected, the map $1\to \Sn n$ selecting the point $N$ is
  $(n-2)$-connected. Hence it suffices to give an element of $P(N)$ to get an
...
TODO:
The following could allow us to simplify the proof of \cref{thm:higher-spheres}.

\begin{lemma}
    Let $X$ be a type and $x : X$ be a point. If $X$ is connected, then the canonical map
    \begin{equation}
        \setTrunc{(X,x) \ptdto (X,x)} \xrightarrow{\setTrunc{\pi_1}} \setTrunc{X \to X}
    \end{equation}
