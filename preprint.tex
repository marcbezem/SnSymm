...
\[
  \begin{tikzcd}
    A \times B \ar[r,"\snd"]\ar[d,"\fst"'] & B \ar[d,"\inr"] \\
    A \ar[r,"\inl"'] & A * B
  \end{tikzcd}
\]
...
\[
  \begin{tikzcd}
    1 \ar[r,"\ast_B"]\ar[d,"\ast_A"'] & B \ar[d,"\inr"] \\
    A \ar[r,"\inl"'] & A \vee B
  \end{tikzcd}
\]
...
\begin{definition}
  Given pointed types $A$ and $B$, there is
  a map $W = W_{A,B} : A * B \to \susp A \vee \susp B$
  making a pushout square with the wedge inclusion $i$:
  \[
    \begin{tikzcd}
      A * B \ar[r,"W"]\ar[d] & \susp A \vee \susp B\ar[d,"i"] \\
      1 \ar[r] & \susp A \times \susp B
    \end{tikzcd}
  \]
\end{definition}
We have $W(\inl a) \jdeq \inr N$, $W(\inr b) \jdeq \inl N$,
and $\ap{W}(\glue(a,b)) = \ap{\inl}(\varphi_A(a)) \cdot \ap{\inr}(\varphi_B(b))$.
