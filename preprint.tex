...
\begin{abstract}
  ...
  has two equivalent connected component and we exhibit members for
  each component.
\end{abstract}
...
$\trp [T] p$. For any dependent function $f\from\prod_{x\from A}T(x)$,
the function
\begin{displaymath}
...
that are defined in the HoTT-book: propositional truncations,
suspensions, and the circle. Considering their importance in this
paper, we recall them in full\footnote{\color{red}Or maybe we should
  make an appendix of that?}:
\begin{enumerate}
\item the {\em propositional truncation} of a type $A$ is a type
  $\Trunc A$ defined by
  \begin{itemize}
  \item a map $\trunc \blank \from A \to \Trunc A$,
  \item and a dependent function
...
\item the {\em suspension} of a type $A$ is a type $\susp A$ defined
  by
  \begin{itemize}
  \item two elements $N,S\from \susp A$,
  \item and a  map $\mrd \from A \to (N=S)$
  \end{itemize}
  with the following induction rule:
  \begin{quote}
...
\item the {\em circle} $\Sc$ is a type defined by
  \begin{itemize}
  \item an element $\base \from \Sc$,
  \item and a path $\Sloop: \base = \base$
  \end{itemize}
  with the following induction rule:
  \begin{quote}
...
exactly two connected components, equivalent one to
another. Determining a type with a simple description that is
...
components, equivalent one to another. Each induction step relies on a 
...
\begin{equation}
  ...
  (\Sc = \Sc) \weq \Sc+\Sc.
\end{equation}
We will work transparently across univalence, so that equivalences
...
\item next, we will prove that these two elements are not in the same
  connected component;
\item then, we will prove that every equivalences in $\Sc=\Sc$ is
