...
\to \Sn n}$ is as a monoid isomorphic to $(\ZZ, \times)$. Then, the argument of
\cref{TODO:refer-to-earlier-result} applies.
...
\end{tikzpicture}
\end{equation}
  \todo[inline]{Pierre: the diagram should be completed with an extra $\Phi^{-1}$ on the right decomposing $\phi^n$.}
...
\todo[inline]{Pierre: choose between the two proofs.}
...
The type $\UUptd$ does certainly not form a category (the intended type of
morphisms $A \ptdto B$ between two objects $A,B$ is not necessarily a set),
but some constructions on it are reminiscent of functors. 
\begin{definition}
  A \emph{wild functor} on $\UUptd$ is a dependent $4$-tuple $(F_0,F_1,c,u)$ where:
  \begin{align*}
    F_0 &:\UUptd \to \UUptd,\\
