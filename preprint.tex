...
The proof of this lemma is by Ulrik Buchholtz, who presented a formalisation in cubical Agda.
Here, we give the argument on paper:
\begin{proof}
	Using function extensionality and assuming $x : \susp(\susp(A))$, we need to construct $h(x) : \susp(-\id_{\susp(A)})(x) = -\id_{\susp(\susp(A))}(x)$.
	...
	&&\color{darkblue}{\defequi} &
	& \color{darkblue}{
		\inv{\left(\mrd_{\susp(\susp(A))}(S_{\susp(A)})\right)} 
	  }
  	&& \color{darkblue}{:} &
	& \color{darkblue}{
		S_{\susp(\susp(A))} = N_{\susp(\susp(A))}
	  }
  	\label{eq:blue}
	\end{alignat}
	
	The last component to be defined is, for all $y : \susp(A)$, a higher path $\ap h(\mrd_{\susp(\susp(A))}(y))$.
	After rewriting path-overs as path composition using a standard lemma \cite[Lem~2.11.2]{HoTT}, the type of this component becomes a commutativity proof for the square
	...
	\draw[-latex,text=darkgreen] (N1) to node[left] {$\mrd_{\susp(\susp(A))}(N_{\susp(A})$} (S1);
	\draw[-latex,text=darkblue] (N2) to node[right] {$\mrd_{\susp(\susp(A))}(S_{\susp(A)})$} (S2);
	\draw[-latex] (N1) to node[above] {$\mrd_{\susp(\susp(A))}(-\id_{\susp(\susp(A))}(y))$} (S2);
	\draw[-latex] (N2) to node[above] {$\mrd_{\susp(\susp(A))}(y)$} (S1);
	\end{tikzpicture}
	\end{equation}
	...
	&m_N \defequi \mrd_{\susp(\susp(A))}(N_{\susp(A}) \\
	&m_S \defequi \mrd_{\susp(\susp(A))}(S_{\susp(A)})
	\end{align}
	...
	\end{tikzpicture}
	\end{equation}
	with the obvious commutativity proof $c(m_S,m_N) : m_S \cdot \inv{m_S} = m_N \cdot \inv{m_N}$ (given $p$, $q$, we define $c(p,q) : p \cdot \inv{p} = q \cdot \inv{q}$ by path induction on $p$ and $q$).
%	 : m_N \cdot \inv{m_N} = m_S \cdot \inv{m_S}$.
	...
	\draw[-latex] (N2) to node[above] {$m_S$} (S1);
	\end{tikzpicture}
	\end{equation}
	with the trivial commutativity proof $\refl{} : m_N \cdot \inv{m_S} = m_N \cdot \inv{m_S}$.
	
	Finally, we consider the third case of the induction on $y$, namely the case that $y$ is a path of the form $\mrd_{\susp(A)}(z)$ for some $z : A$.
	In this case, we need to show an equality between $p$ and $\refl{}$ (the ``fillers'' of the two squares \eqref{eq:N-commute} and \eqref{eq:S-commute}), and this equality lies over (i.e.\ depends on) the two equalities that stem from the top horizontal arrow and the bottom horizontal arrow in \eqref{eq:y-commute} (note that the two vertical arrows in \cref{eq:y-commute} do not depend on $y$).
	We write $\alpha$ for the top horizontal arrow in \eqref{eq:y-commute} with $y 
	\equiv \mrd_{\susp(A)}(z)$,
	\begin{equation}\label{eq:alpha-def}
	\alpha \defequi \ap{\mrd_{\susp(\susp(A))}}\left(\ap{-\id_{\susp(\susp(A))}}(\mrd_{\susp(A)}(z))\right)
	\end{equation}
	of type $m_S = m_N$, and $\beta$ for the bottom horizontal arrow, i.e.\
	\begin{equation}
	\beta \defequi \ap{\mrd_{\susp(\susp(A))}}(\mrd_{\susp(A)}(z)))
	\end{equation}
	of type $m_N = m_S$.
	Writing path overs as composition as before, the first square $c(m_S,m_N)$ becomes the composition
	\begin{equation}\label{eq:first-square-composed}
	(\alpha^{-1} \cdot \inv{m_S}) \star c(m_S,m_N) \star (m_N \cdot \ap{\inv{}}\beta)
	\end{equation}
	of type $m_N \cdot \inv{m_S} = m_N \cdot \inv{m_S}$.
	In \eqref{eq:first-square-composed}, $\cdot$ denotes whiskering and $\star$ horizontal composition. The situation can be depicted as follows:
	...
	\end{tikzpicture}
	\end{equation}
	The goal is now to show that \eqref{eq:first-square-composed} is equal to the trivial proof $\refl{}$.
	Rewriting the definition \eqref{eq:alpha-def} using standard path algebra steps, we get $\alpha = \inv{\beta}$, and \eqref{eq:first-square-composed} simplifies to
	\begin{equation}\label{eq:whiskered-square}
	(\beta \cdot \inv{m_S}) \star c(m_S,m_N) \star (m_N \cdot \ap{\inv{}}\beta).
	\end{equation}
	As suggested by Ulrik Buchholtz, the easiest way to show that this expression is trivial is to generalise to arbitrary points $N$, $S$ and arbitrary paths $m_N$, $m_S$ as well as an arbitrary higher path $\alpha$. Path inductions on $m_N$ and $\alpha$ replace all paths including $c(m_S,m_N)$ in the expression by $\refl{}$, and the equality of \eqref{eq:whiskered-square} and $\refl{}$ becomes obvious.
\end{proof}
...
\begin{corollary}\label{cor:id-not-minus-id}
	For any $n$, we have $\id_{\Sn n} \neq -\id_{\Sn n}$.
\end{corollary}
\begin{proof}
	\todo[inline]{TODO: complete (N.)}
\end{proof}
...
\begin{proof}
    ...
    \todo[inline]{Nicolai: Need to fill in the argument for $n>2$. Something like $\susp$ preserves equivalence of components.
    
    Pierre: still missing that piece. Basically, one would like $\Trunc{\susp f = \susp g} \to \Trunc{f = g}$ and I am just able to prove that $\Trunc{\prod_x\mrd(f(x)) = \mrd(g(x))}$. There is no reason for $\mrd$ to have a retraction in full generality (or at least I recall that it as the conclusion we reached with Marc after a brainstorming session).

	Nicolai: TODO: follows from \cref{prop:equiv-susp-comp} and \cref{cor:id-not-minus-id}}
\end{proof}
