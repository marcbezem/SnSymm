...
\subsection{Connectedness of the H-Monoid Morphism}\todo[inline]{"H'' or ``wild'', decide and discuss this in the introduction. 

Pierre: I vote for $H$, it catch the eye more than a mere adjective.}
...
\todo[inline]{As far as I know there is no lemma in the HoTT-book, only 6.5.4 describing the equivalence of \cref{eq:adj-equivalence-susp-loopspace}. In particular, I don't see the naturality of this equivalence anywhere in the book. }
...
\Phi$\todo{$\Phi$ probably need some subscript}.
...
\begin{theorem}
    ...
    \begin{equation}
    \pi_1(\Sn n = \Sn n) = \ZZ \slash 2\ZZ
    \end{equation}
    as groups.\todo{We don't really show this, unfortunately. We only show equality as types, where the second thing is just Bool. BUT there is only one group structure anyway, and it's clear what the identity is.}
\end{theorem}
\begin{proof}
    ...
    \begin{equation} \label{eq:3-coherator}
    \begin{tikzpicture}[baseline=(current bounding box.center)]
    \node (P1) at (\rot+0:\R) {$\Omega^{n+1}(\Sn n)$}; 
    \node (P2) at (\rot+60:\R) {$\Omega(\Omega^{n}(\Sn n))$}; 
    \node (P3) at (\rot+120:\R) {$\Omega(\Sn n \to_* \Sn n)$}; 
    \node (P4) at (\rot+180:\R) {$\Omega(\Sn n \to \Sn n)$}; 
    \node (P5) at (\rot+240:\R) {$\Omega(\Sn n \simeq \Sn n)$}; 
    \node (P6) at (\rot+300:\R) {$\Omega(\Sn n = \Sn n)$}; 

    \draw[<->, tikzshortarrow] (P1) to node [above left] {$\sim$} (P2);
    \draw[<-, tikzshortarrow] (P2) to node [left] {$\Omega(\Phi^n)$} (P3);
    \draw[->, tikzshortarrow] (P3) to node [above left] { $\Omega(\pi_1)$} (P4);
    \draw[<->, tikzshortarrow] (P5) to node [right] {$\sim$} (P6);
    \draw[->, tikzshortarrow] (P5) to node [below right] {$\Omega(\pi_1)$} (P4);
    \end{tikzpicture}
    \end{equation}
    \todo[inline]{todo}
\end{proof}
