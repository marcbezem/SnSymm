...
transport
$\trp[T]{\alpha}$
%:(\trp[\hopffam]{e}(\base) = \base) \to (\trp[\hopffam]{e'}(\base) =\base)$ 
identifies (extensionally) with the following function:
\begin{equation}
  \begin{aligned}
...
% $-\id_\Sp$, one still needs to prove that equivalences have degree
...
equivalences $\Sc \ptdto \Sc$ are equal to either $(\id_\Sc,\refl\base)$ or
$(-\id_\Sc,\refl\base)$.\footnote{\color{red}This only appears in the proof of the
...
is a pointed equivalence $\Sc \ptdto \Sc$ which is already known to be
